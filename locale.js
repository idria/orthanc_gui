exports.es = {
    invalidConfiguration: "Archivo de configuración inexistente o inválido!",
    invalidNew: "No se puede crear archivo de configuración nuevo.",
    connectionError: "Error de conexión: ",
    sendError: "Error de envio: ",
    invalidResponse: "Respuesta inválida!",
    patientName: "Nombre",
    accessionNo: "No. de Acceso",
    accessionNoLabel: "Cambiar No. de Acceso:",
    patientId: "ID de Paciente",
    patientIdLabel: "Cambiar ID de Paciente:",
    studyDate: "Fecha",
    studyDateFormat: "(DD/MM/AAAA)",
    modality: "Modalidad",
    all: "Todos",
    search: "Buscar",
    clean: "Borrar",
    dest: "Destino",
    desc: "Descripción",
    send: "Enviar",
    open: "Abrir",
    split: "Separar",
    empty: "Complete Nombre, ID, No. de Acceso y/o Fecha!",
    delete: "Borrar",
    errChange: "No se puede modificar el campo!",
    ret: "Volver",
    seriesNo: "No. de Series",
    seriesDesc: "Descripción de Series",
    bodyPartExamined: "Parte del Cuerpo",
    stationName: "Nombre de Estación",
    noSelection: "Nada seleccionado!",
    onlyOneSelection: "Solo puede borrar un item por vez.",
    splitMessage: "1. Verifique los datos del estudio.</br>2. Seleccione las series que desea separar a un nuevo estudio.</br>3. Click en separar.</br>4. Ingrese el Número de Acceso del estudio nuevo.",
    modified: "Modificado!",
    deleted: "Borrado!",
    deleteQuestion: "Desea borrar el estudio seleccionado?",
    deleteQuestionSeries: "Desea borrar la serie seleccionada?",
    errModifyPatient: "No se puede modificar el paciente. Tiene enlazado mas de un estudio.",
    exportLabel: "Exportar",
    series: "Series",
    auditConnectionError: "No hay conexión con el servidor de auditoria.",
    sendStudyConfirm: "Desea enviar el estudio seleccionado a: ",
    protocolName: "Nombre de Protocolo",
    seriesDate: "Fecha de Series",
    seriesTime: "Hora de Series",
};

exports.en =  {
    invalidConfiguration: "Missing or invalid configuration file!",
    invalidNew: "Can't create new configuration file.",
    connectionError: "Connection error: ",
    sendError: "Send error: ",
    invalidResponse: "Invalid response!",
    patientName: "Name",
    accessionNo: "Accession No.",
    accessionNoLabel: "Change Accession No.:",
    patientId: "Patient ID",
    patientIdLabel: "Change Patient ID:",
    studyDate: "Date",
    studyDateFormat: "(DD/MM/YYYY)",
    modality: "Modality",
    all: "All",
    search: "Search",
    clean: "Clean",
    dest: "Destination",
    desc: "Description",
    send: "Send",
    open: "Open",
    split: "Split",
    empty: "Fill Name, ID, Accession No. and/or Date!",
    delte: "Delete",
    errChange: "Can't change field!",
    ret: "Return",
    seriesNo: "Series No.",
    seriesDesc: "Series Description",
    bodyPartExamined: "Body Part",
    stationName: "Station Name",
    noSelection: "Nothing selected!",
    onlyOneSelection: "Can delete only one item per time.",
    splitMessage: "1. Verify study data.</br>2. Select series to split in a new study.</br>3. Click split.</br>4. Enter the new study Accession Number.",
    modified: "Modified!",
    deleted: "Deleted!",
    deleteQuestion: "Would you like to delete selected study?",
    deleteQuestionSeries: "Would you like to delete selected series?",
    errModifyPatient: "Can't modify patient. Has more than one study linked.",
    exportLabel: "Export",
    series: "Series",
    auditConnectionError: "No connection to audit server.",
    sendStudyConfirm: "Do you want to send the selected study to: ",
    protocolName: "Protocol Name",
    seriesDate: "Series Date",
    seriesTime: "Series Time",

};